<script lang="ts">
  ...
  let { values, axisLabels = [] }: { values: number[]; axisLabels?: (string | null)[] } = $props()
    ...
    c.set({
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          return `λ<sub>${p.dataIndex}</sub> = ${adaptive(p.value)}`
        },
      },
...
<div
  bind:this={node}
  role="img"
  aria-label="Spectrum of {values.length} selected eigenvalues, relative to the largest absolute eigenvalue. View values below."
  class="h-full w-full"
></div>
