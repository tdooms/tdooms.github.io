...
/**
 * Top 32 eigenvalues by |λ|, normalised so the largest absolute bar is 1
 ...
 * manifold scene's three axes (the top three by |λ|), else ``null``.
 */
export const topSpectrum = (eigvals: number[]): { values: number[]; labels: (string | null)[] } => {
  const indexed = eigvals.map((v, i) => [v, i] as const)
  indexed.sort((a, b) => Math.abs(b[0]) - Math.abs(a[0]))
  const top = indexed.slice(0, 32).sort((a, b) => a[0] - b[0])
  const axisIdx = indexed.slice(0, 3).map(([, i]) => i)
  ...
  return {
    ...
    labels: top.map(([, i]) => {
      ...
      // j is a position in axisIdx, which contains at most three axes.
