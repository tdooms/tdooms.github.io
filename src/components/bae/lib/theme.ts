...
const toHex = (css: string): string => {
  ...
  if (!_ctx) throw new Error('A 2D canvas context is required to resolve Atlas colours')
  _ctx.fillStyle = '#fff' // fallback if css is invalid
  _ctx.fillRect(0, 0, 1, 1)
  _ctx.fillStyle = css
  _ctx.fillRect(0, 0, 1, 1)
  const pixel = _ctx.getImageData(0, 0, 1, 1).data
  // A one-pixel ImageData has four RGBA entries.
  return `#${hex(pixel[0]!)}${hex(pixel[1]!)}${hex(pixel[2]!)}`
}

const cssVar = (name: string): string =>
  toHex(getComputedStyle(document.documentElement).getPropertyValue(`--color-${name}`).trim())
...
// Layout owns the persisted choice on <html>. Canvas renderers need resolved
// colours again when that choice changes; CSS alone cannot recolour pixels.
// The first subscriber reads synchronously, and the last one disconnects the
// shared observer so navigating away leaves no theme listener behind.
export const theme = readable<Theme>(undefined, (set) => {
  const update = () => set(readTheme())
