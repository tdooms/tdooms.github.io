...
outputs, and weights. Any slight change in input can cause a ReLU to activate, so the only way to compute a ReLU's output
...
The natural objection: if each branch is just linear, how can this be a good model component? While either side is linear, the whole is bilinear, which is non-linear with respect to the input. That's all we need.

<figure class="text-center">
  <Image
    src={comparison}
    alt="Three common MLPs: A ReLU, a bilinear MLP and a SwiGLU"
    class="inline-block w-full max-w-md"
  />
  <figcaption>
    An swiGLU has a gate part which "selects" which parts of the other side should be kept. Bilinear
    layers simply omit this gate and keep continuous interactions instead.
  </figcaption>
</figure>
...
Below: the most important eigenvectors of a single-layer MNIST model. Switch the target digit, augmentation, and eigenvector index to see how the decomposition responds.

<Eigenvectors client:visible />

Eigenvectors have a different interpretation than heatmaps. The contribution toward the output is strictly defined by whether the eigenvalue is positive or negative; strokes of the same colour positively interfere while strokes of opposite colour negatively interfere. Many strokes can be read as localised edge detectors. Positive eigenvectors show structure that corresponds to important strokes or proto-digits; negative eigenvectors show inhibitory strokes that would adversely affect the classification.
