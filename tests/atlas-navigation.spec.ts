...
test('Atlas loads composite code and vocabulary only after selecting a composite', async ({
  ...
  const stats = page.getByRole('banner').getByRole('group', { name: 'Composite statistics' })
  await expect(stats.getByRole('button', { name: 'density 0.10', exact: true })).toBeVisible()
  ...
  await expect(stats.getByRole('button', { name: 'density 0.40', exact: true })).toBeVisible()
  await expect(stats.getByRole('button', { name: 'importance 0.60×', exact: true })).toBeVisible()
  expect(vocabRequests).toBe(1)
})
...
test('Atlas search results are visible and navigable on a phone', async ({ page }) => {
  ...
  await expect(page).toHaveTitle(/^Atlas · .* · Alpha contexts$/)
  await expect(page.locator('canvas[aria-label*="3D scatter"]')).toBeVisible()
